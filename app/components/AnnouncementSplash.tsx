      ...
      >
        ...
        <div className="flex items-center justify-between px-8 pt-6 pb-2">
          <div className="flex items-center gap-3">
            <div className={`w-3 h-3 rounded-full animate-pulse ${isCommercial ? "bg-amber-400" : "bg-rose-400"}`} />
            <span className={`text-xs font-mono tracking-[0.3em] uppercase font-bold ${isCommercial ? "text-amber-400" : "text-rose-300"}`}>
              {isCommercial ? "🛒 Оголошення про продаж" : "💌 Програма знайомств"}
            </span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-8 pb-6">
          ...
          {isCommercial && announcement.itemTitle && (
            <div className="mb-4">
              <h2 className="text-amber-300 text-2xl font-bold leading-tight">{announcement.itemTitle}</h2>
              <div className="flex flex-wrap gap-4 mt-2 text-sm text-amber-500 font-mono">
                ...
                {announcement.contactPhone && <span>📞 {announcement.contactPhone}</span>}
              </div>
            </div>
          )}
          ...
          {/* AI text — big */}
          {announcement.aiText && (
            <blockquote className={`text-lg md:text-xl leading-relaxed italic font-serif ${isCommercial ? "text-amber-100" : "text-rose-100"}`}>
              «{announcement.aiText}»
            </blockquote>
          )}
        </div>
        ...
        {/* Footer */}
        <div className={`px-8 py-4 flex items-center justify-between border-t ${isCommercial ? "border-amber-900 bg-amber-950/60" : "border-rose-900 bg-rose-950/60"}`}>
          <span className="text-xs text-gray-500 font-mono">Подати власне оголошення → /submit</span>
          <button
            onClick={onClose}
            className={`px-5 py-2 rounded-xl text-sm font-semibold transition ${isCommercial
